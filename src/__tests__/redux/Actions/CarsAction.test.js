...
describe('Async Actions', () => {
  ...
  describe('Load Cars Thunk', () => {
    it('should create SET_CARS when loading CAR', () => {
      fetchMock.mock('*', {
        body: cars,
        headers: { 'content-type': 'application/json', Authorization: 'Bearer jkgjdlfdlfkl' },
      });

      const expectedActions = [{ type: types.SET_CARS, cars }];

      const store = mockStore({ cars: [] });
      return store.dispatch(CarsActions.loadCars('a')).then(() => {
...
describe('Meals Action Creator', () => {
  it('should create a SET_MEALS action', () => {
    // arrange
    const cars = null;
    const expectedAction = { type: types.SET_CARS, cars };
